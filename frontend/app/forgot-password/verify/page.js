...
function VerifyForm() {
  ...
  const handleChange = (index, value) => {
    ...
    if (filled.length === 6 && /^\d{6}$/.test(filled)) {
      setTimeout(() => submitOtp(filled), 150);
    }
  };
  ...
  const submitOtp = async (code) => {
    if (!email) return setError('No email provided. Please start over.');
    if (code.length !== 6) {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'var(--bg)' }}>
      ...
      <div className="auth-container">
        <div className="auth-card">
          ...
          <p className="auth-subtitle" style={{ wordBreak: 'break-all' }}>Verification code sent to your primary email {sentTo}</p>
