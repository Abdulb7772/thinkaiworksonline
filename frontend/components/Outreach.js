...
const campaigns = [
  {name:'Amazon Sellers — FBA Mgmt',co:'es',sent:186,opens:'44%',replies:'9%',status:'active'},
  {name:'Shopify Stores — Scaling',co:'es',sent:94,opens:'38%',replies:'7%',status:'active'},
  {name:'SaaS Companies — AI Tools',co:'tai',sent:106,opens:'51%',replies:'12%',status:'active'},
];

export default function Outreach({ company, onToast }) {
  const metrics = [
    { label: 'Prospects Found', val: '1,240', delta: 'Upwork + LinkedIn', cls: 'neutral', co: 'es' },
    { label: 'Emails Sent', val: '386', delta: 'This week', cls: 'up', co: '' },
    { label: 'Open Rate', val: '44%', delta: '\u2191 vs 22% avg', cls: 'up', co: '' },
    { label: 'Booked Calls', val: '23', delta: '\u2191 9% rate', cls: 'up', co: '' },
  ];
  ...
  return (
    <div className="page active" style={{display:'flex',flexDirection:'column',gap:22}}>
      <div className="ph">
        ...
        <button className="btn btn-es" onClick={() => onToast('New outreach campaign queued · AI personalizing 89 emails \u2713','success')}>{'\u26A1'} Launch Campaign</button>
      </div>
      <div className="grid4">
        {metrics.map((m,i) => (
